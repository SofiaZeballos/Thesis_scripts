...
install.packages('compositions', quietly=TRUE)
...
BiocManager::install(c("GEOquery", "Biobase", "tximport", "EnsDb.Hsapiens.v86", "EnsDb.Rnorvegicus.v79", "EnsDb.Mmusculus.v79",
...
my.fastqs_1 <- my.fastqs[grep("_pass_1.fastq.gz$", my.fastqs)]

# List for _pass_2.fastq.gz files
my.fastqs_2 <- my.fastqs[grep("_pass_2.fastq.gz$", my.fastqs)]
...
indices_ordenados <- order(colnames(fc$counts))

# Ordenar las columnas de la matriz
fc$counts <- fc$counts[, indices_ordenados]
head(fc$counts,2)
getwd()

#save to an external file
write.table(fc$counts, file="..\\Archivos\\Datasets_normalizados\\kalesky_experiment_gene-count.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)


#save to an RDS format object
saveRDS(fc, file="..\\Archivos\\Datasets_normalizados\\kalesky_experiment_fc.Rds")

# ----- 3. Normalizacion -----

path_samples = '..\\Archivos\\Tablas_conteo\\kalesky_info_samples.csv'
...
wb_seqname <- genes.df$entrezgene_id
...
rownames(genes.df) <- genes.df$entrezgene_id
...
y <- DGEList(counts=fc$counts_filtrado, samples=targets, genes=genes.df, group=targets$tissue)
...
#Es clara la división entre los diferentes tejidos
...
counts_normalized <- y$counts_normalized

sum(row.names(counts_normalized) %in% c("", NULL, NA))

# Initialize an empty matrix for the averages
y$counts_norm_prom <- matrix(0, nrow = nrow(counts_normalized), ncol = length(groups))
rownames(y$counts_norm_prom) <- rownames(counts_normalized)
head(y$counts_norm_prom)

# Calculate and fill in the averages for each group
for (i in seq_along(groups)) {
  group <- groups[i]
  group_cols <- grep(group, colnames(counts_normalized))
  y$counts_norm_prom[, i] <- rowMeans(counts_normalized[, group_cols], na.rm = TRUE)
}

# Assign column names to the new matrix
colnames(y$counts_norm_prom) <- groups

head(y$counts_norm_prom)
dim(y$counts_norm_prom)

#save to an external file
write.table(y$counts_norm_prom, file="..\\Archivos\\Datasets_normalizados\\kalesky_et_al_experiment_gene-count_normalized_TMM.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
...
write.table(y$counts_norm_prom_tpm, file="..\\Archivos\\Datasets_normalizados\\kalesky_et_al_experiment_gene-count_normalized_TPM.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
...
#save to an external file
write.table(y$counts_norm_prom_tpm_clr, file="..\\Archivos\\Datasets_normalizados\\kalesky_et_al_experiment_gene-count_normalized_TPMCLR.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)

saveRDS(y, file="..\\Archivos\\Tablas_conteo\\kalesky_et_al_experiment_y.Rds")
