...
#Tengo que sacarle el "CELE_:" de las filas para después poder usar esos identificadores
...
write.table(fc$counts, file="..\\Archivos\\Datasets_normalizados\\Mirza_experiment_gene-count.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)

#save to an RDS format object
saveRDS(fc, file="..\\Archivos\\Tablas_conteo\\Mirza_experiment_fc.Rds")

# ----- 3. Normalizacion -----

path_samples = '..\\Archivos\\Tablas_conteo\\Mirza_info_samples.csv'
...
group.c <- as.factor(y$samples$Group)
group.c
# group.c <- relevel(group.c, ref="Ecoli_15")

pch <- c(15:23)
colors <- rep(c("blue", "red", "green", "purple"), each = 3)

plotMDS(y, col=colors, pch=16, cex=2)
# Añadir la leyenda fuera del gráfico
legend('bottom', legend=levels(group.c), pch=16, col=unique(colors), ncol=2)
#Es clara la división entre ecoli y bsub
...
#save to an external file
write.table(y$counts_norm_tmm_prom, file="..\\Archivos\\Datasets_normalizados\\mirza_et_al_experiment_gene-count_normalized_TMM.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
...
getwd()
write.table(y$counts_norm_prom_tpm, file="..\\Archivos\\Datasets_normalizados\\mirza_et_al_experiment_gene-count_normalized_TPM.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)
...
getwd()
#save to an external file
write.table(y$counts_norm_prom_tpm_clr, file="..\\Archivos\\Datasets_normalizados\\mirza_et_al_experiment_gene-count_normalized_TPMCLR.tab", 
            quote=FALSE, sep="\t", row.names=TRUE, col.names=TRUE)

#save to an RDS format object
saveRDS(y, file="..\\Archivos\\Tablas_conteo\\mirza_et_al_experiment_y.Rds")
